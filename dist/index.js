...
bot.command('start', async (ctx) => {
    try {
        ...
        if (!user) {
            ...
            const userDataValidation = validateUserData(ctx.from?.first_name || '', ctx.from?.last_name, ctx.from?.username);
        ...
        await ctx.reply(`Вас приветствует телеграм-бот ЖСК Западный г. Москва по приему показаний счетчиков воды\n\n` +
            `Для регистрации в системе используйте команду /register`);
    }
    catch (error) {
        await handleError(ctx, error, 'обработка команды /start');
    }
});
...
bot.command('register', async (ctx) => {
    try {
        ...
        await handleError(ctx, error, 'регистрация пользователя');
    }
});
...
bot.command('submit_readings', async (ctx) => {
    try {
        ...
        if (!ctx.message?.text) {
            await ctx.reply('Для отправки показаний используйте команду в формате: /submit_readings ХХХХ ХХХХ\n' +
        ...
        user.meterReadings.push(meterReading._id);
        await user.save();
        await ctx.reply(`Показания успешно сохранены!\n` +
            `Холодная вода: ${coldWater}\n` +
            `Горячая вода: ${hotWater}\n` +
            `Дата снятия: ${new Date().toLocaleDateString('ru-RU')}`);
    }
    catch (error) {
        await handleError(ctx, error, 'ввод показаний счетчиков');
    }
});
...
bot.command('history', async (ctx) => {
    try {
        ...
        await handleError(ctx, error, 'получение истории показаний');
    }
});
...
const handleError = async (ctx, error, action) => {
